    ...
    <div className="nav-wrapper flex justify-center fixed top-5 w-full z-[1000]">
      ...
      >
        ...
        <nav className="flex gap-1">
          {SECTIONS.map(({ id, label }) => (
            <button
              key={id}
              type="button"
              className={`nav-btn px-5 py-2.5 rounded-[20px] font-semibold text-sm cursor-pointer transition-colors duration-200 border-0 ${
        ...
        >
          ...
          <div
            className="user-pill text-xs font-bold py-2 px-4 rounded-[20px] border"
            style={{
